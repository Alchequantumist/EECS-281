...
public:
	...
	traderId(t), stockId(s), price(p), quantity(q), time(timeStamp), tCount(tCount){}
		...
		if (isT) {
			...
			for (unsigned int i = 0; i < tt.size(); i++) {
				if (tt[i].solSellPrice > tt[i].solBuyPrice) {
					cout << "A time traveler would buy shares of Stock "
						<< i << " at time: " << tt[i].solBuyTime
						<< " and sell these shares at time: " << tt[i].solSellTime << '\n';
				}
				else {
					cout << "A time traveler would buy shares of Stock "
						<< i << " at time: " << -1
						<< " and sell these shares at time: " << -1 << '\n';
				}
			}
		}
	}
